...
export function GuideSearch({ pages }: { pages: SearchPage[] }) {
	...
	return (
		<>
			<Button type="button" variant="outline" className="h-10 w-full justify-between rounded-xl bg-background/70 px-3 text-muted-foreground" onClick={() => setOpen(true)} aria-label="Search the guide">
			...
			{open ? (
				<div className="fixed inset-0 z-[70] flex items-start justify-center bg-foreground/35 px-4 pt-[15vh] backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Search the guide" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
					<div className="w-full max-w-xl overflow-hidden rounded-2xl border border-border bg-background shadow-2xl">
						<div className="flex items-center gap-2 border-b border-border px-4">
							...
							<Input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search sections, topics, and notes..." className="h-14 border-0 px-1 text-base shadow-none focus-visible:ring-0" />
						...
						<div className="max-h-[52vh] overflow-y-auto p-2">
							{results.length ? results.map((page) => (
								<Link key={page.route} href={page.route} onClick={closeSearch} className="block rounded-xl px-3 py-3 transition-colors hover:bg-muted">
